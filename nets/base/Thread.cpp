...
namespace nets::base
{
	...
	Thread::Thread(ThreadFunc threadFunc, const std::string& threadName)
		: runnable_(false), joinable_(false), threadId_(0), tid_(0), threadName_(threadName),
		  threadFunc_(std::move(threadFunc)), latch_(1)
	{
	}
	...
	namespace
	{
		typedef struct ThreadRouter_
		{
		public:
			ThreadRouter_(const ::std::string& threadName, pid_t* tid, Thread::ThreadFunc threadFunc, CountDownLatch& latch)
				: threadName_(threadName), tid_(tid), threadFunc_(::std::move(threadFunc)), latch_(latch)
			{
			}

			~ThreadRouter_() = default;
