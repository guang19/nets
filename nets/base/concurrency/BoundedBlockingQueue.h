    ...
    namespace base
    {
        ...
        class BoundedBlockingQueue : Noncopyable
        {
            ...
            public:
                ...
                void take(ReferenceType el);

				template<typename Predicate>
				bool put(ConstReferenceType el,  Predicate p);
                template<typename Predicate>
				bool take(ReferenceType el, Predicate p);

                bool put(ConstReferenceType el, ::std::time_t milliseconds);
                bool take(ReferenceType el, ::std::time_t milliseconds);

                template<typename Predicate>
				bool put(ConstReferenceType el, ::std::time_t milliseconds, Predicate p);
                template<typename Predicate>
				bool take(ReferenceType el, ::std::time_t milliseconds, Predicate p);

                bool tryPush(ConstReferenceType el);
                bool tryPop(ReferenceType el);
		...
		template<typename T, typename Container>
        void BoundedBlockingQueue<T, Container>::take(ReferenceType el)
        {
			LockGuardType lock(mutex_);
			while (queue_.empty())
			{
		...
		template<typename T, typename Container>
		template<typename Predicate>
		bool BoundedBlockingQueue<T, Container>::take(ReferenceType el, Predicate p)
		{
			LockGuardType lock(mutex_);
		...
		template<typename T, typename Container>
        bool BoundedBlockingQueue<T, Container>::take(ReferenceType el, ::std::time_t milliseconds)
        {
			LockGuardType lock(mutex_);
			if (queue_.empty())
			{
		...
		template<typename T, typename Container>
		template<typename Predicate>
		bool BoundedBlockingQueue<T, Container>::take(ReferenceType el, ::std::time_t milliseconds, Predicate p)
		{
			LockGuardType lock(mutex_);
			if (queue_.empty() && !p())
			{
		...
		template<typename T, typename Container>
        bool BoundedBlockingQueue<T, Container>::tryPop(ReferenceType el)
		{
			LockGuardType lock(mutex_, TRY);
			if (lock.isLockedByCurrentThread() && !isFull())
			{
